...
export function checkAuthCookie () {
  return dispatch => {
    let authResult = cookies.get('is_authenticated')
...
export function savePersonalisationValues (values) {
  ...
  return (dispatch, getState) => {
    ...
    if (isLoggedIn && values.length) {
      ...
      cookies.set('savedValues', JSON.stringify(newValues), { path: '/', secure: true, maxAge: 60 * 60, expires: soon })
    }
  }
}
...
export function fetchPersonalisationValues () {
  return (dispatch, getState) => {
    let oldData = cookies.get('savedValues')
    ...
    if (getState().personalisation.isLoggedIn) {
      ...
      return fetch('/api/users/me/', {
        ...
        .then(json => {
          ...
          cookies.remove('savedValues', { path: '/' })
