...
export class MyProfile extends Component {
  ...
  UNSAFE_componentWillReceiveProps (nextProps) {
    ...
    if (!this.props.isLoggedIn && nextProps.isLoggedIn) {
      // display login prompt after user logges in
      this.setState({ isJustLoggedIn: true })
    } else if (this.props.isLoggedIn && nextProps.isLoggedIn && this.props.isScriptLoaded) {
      // remove login prompt after props changed ( /user s done something)
      this.setState({ isJustLoggedIn: false })
    }
  }
  ...
  saveDueDate () {
    if (this.state.dueDateFieldValue) {
      if(this.dueDateValidate()) {
        let { settings } = this.props.personalisationValues
        if (!settings || this.state.dueDateFieldValue !== settings.dd) {
          let valuesToSave = ([{ 'group': 'settings', 'key': 'dd', 'val': this.state.dueDateFieldValue }])
  ...
  dueDateChange (event) {
    ...
    this.setState({ dueDateFieldValue: event.target.value }, this.saveDueDate)
  }
          ...
          {!this.props.isLoggedIn &&
            <div className='login-section'>
              <div className={this.props.isLoggedIn ? 'hidden' : ''}>
                <h4>Please login to sign up or update your details</h4>
              </div>
              <PrimaryLogin afterLoginAction="open-profile" clickSource="drawer"/>
            </div>
          }
