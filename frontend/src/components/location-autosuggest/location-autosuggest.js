    ...
    if (typeof onPlaceSelect === 'function') {
      this.placeService.getDetails({ placeId: suggestion.placeId }, (place, status) => {
        if (status == google.maps.places.PlacesServiceStatus.OK) {
          // sometimes google returns wrong street number (even though we query on placeId)
          // e.g. we selected 5b Picton, but get back 5 Picton
          // to prevent it, select first part of suggestion till comma as a street name
          const placeName = suggestion.name.split(',')[0]
          onPlaceSelect(place, placeName)
        }
      })
    }
  }
