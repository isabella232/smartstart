...
import { fetchSchema, findAreaByPostCode, fetchThresholds, postToReasoner } from 'actions/entitlements'
...
class EntitlementsQuestions extends Component {
  ...
  submit (values) {
    const { schema, postToReasoner, postCode, thresholds } = this.props
    ...
    // if we have postcode we should find an corresponding area first,
    if (postCode) {
      findAreaByPostCode(postCode).then(areaCode => {
        // add area to values ( but doesn't have to have it on a form )
        postToReasoner(transform(values, schema, thresholds, areaCode))
        this.props.router['push']('/financial-help/results')
      }).catch(() => {
        // still procced to results page even if can't get the area
        postToReasoner(transform(values, schema, thresholds, null))
        this.props.router['push']('/financial-help/results')
      })
    } else {
      // add area to values ( but doesn't have to have it on a form )
  ...
  render () {
    ...
    if (fetchingSchema || fetchingThresholds) {
            ...
            { hasAccommodationCosts === 'true' &&
              <div className='component-grouping'>
                ...
                { hasSocialHousing === 'false' &&
                  <div className='component-grouping'>
                    <Field {...getFieldProps(fields, 'applicant.postCode')} />
                  </div>
                }
              </div>
            }
...
EntitlementsQuestions.propTypes = {
  ...
  fetchingThresholds: PropTypes.bool,
  findAreaByPostCode: PropTypes.func,
  ...
  hasSocialHousing: PropTypes.string,
...
const mapStateToProps = (state) => ({
  ...
  fetchingThresholds: get(state, 'entitlements.fetchingThresholds'),
  thresholds: get(state, 'entitlements.thresholds'),
  ...
  hasSocialHousing: selector(state, 'applicant.hasSocialHousing'),
  postCode: selector(state, 'applicant.postCode')
})
