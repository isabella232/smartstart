...
import validateStep7 from './steps/validation/step7'

let steps, stepValidateFn
...
if (MYIR_ENABLED) {
  ...
  stepValidateFn = {
    ...
    '4': validateStep4,
    '5': validateStep5,
    '6': validateStep7 // mismatch because one of the steps disabled
  }
...
class FormWizardProgress extends Component {
  ...
  navigateToStep(targetStep) {
    ...
    const isCurrentStepComplete = currentStep === steps.length + 1 || isComplete(formState, currentStep)
  ...
  render() {
    ...
    const reviewPageNumber = steps.length + 1
    return (
      <div className="form-wizard-progress">
        ...
        <StepProgress
          step={reviewPageNumber}
          isComplete={false}
          isClickable={everyStepComplete}
          isCurrent={currentStep === reviewPageNumber}
          onStepClick={this.navigateToStep}
        />
      </div>
    )
