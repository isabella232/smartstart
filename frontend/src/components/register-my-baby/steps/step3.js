...
class FatherDetailsForm extends Component {
  ...
  onPlaceSelect(placeDetail, placeName) {
    const { address_components: addressComponents } = placeDetail

    const streetAddress = placeName || get(placeDetail, 'name', '')
  ...
  render() {
    const {
      ...
      secondParentPreferedTitle,
      fatherKnown, handleSubmit, submitting
    } = this.props

    const secondParentTitle = getSecondParentTitle(assistedHumanReproductionWomanConsented, assistedHumanReproductionManConsented, secondParentPreferedTitle, fatherKnown)
    ...
    return (
      <div>
        <h2 className="step-heading">
          ...
          <span className="english">{capitalizeWordsAll(secondParentTitle)}</span>
        </h2>
        <SaveAsDraft step={3} />
          ...
          { assistedHumanReproduction === 'yes' &&
            <div className="conditional-field">
              <div className="instruction-text">
                Select one of the following to describe your relationship.  The option you select will change how the other parent details are completed.
              </div>
              <Field
              ...
              />
              {assistedHumanReproductionWomanConsented && <Field {...getFieldProps(schema, 'secondParent.preferedTitle')}/> }
              <Field
