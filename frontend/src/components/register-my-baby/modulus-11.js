...
/**
  These validation rules come from Payments NZ
  1) The account structure is: 2 digit Bank Number 4 digit branch 6 digit account 2 digit suffix.
    a. Note, accounts are actually 7 digits, but exclude the first 0 from the validation
  2) The account check digits are performed over the four digits of the Branch Number and six digits
    of the account number (excludes the left most ‘0’ of the account number).
  3) The weightings for the calculation are from left to right: 6, 3, 7, 9, 10, 5, 8, 4, 2, 1.
**/
...
export const validateBankAccountNumber = value => {
  ...
  if (isNaN(numberValue) || (stringValue.length !== 15 && stringValue.length !== 16)) {
    return false
  }

  // Get 4 digit branch number and 7 digit account number - first 0 of account number excluded
  const branch = stringValue.substring(2, 6)
  const account = stringValue.substring(7, 13)
  const base = branch + account

  const parts = base.split('')

  const firstCalculatedCheckDigit = calculateCheckDigit(parts, [6, 3, 7, 9, 10, 5, 8, 4, 2, 1])
  if (firstCalculatedCheckDigit !== 10) {
    return firstCalculatedCheckDigit === 0
  }
  return false
}
